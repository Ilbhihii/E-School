...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-sky-100 p-6">
    ...
    <div class="w-full max-w-2xl bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/40 p-8">
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-indigo-600">Modifier le Live</h1>
            <p class="text-gray-500 text-sm">Mettre à jour les informations du live</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.lives.update', $live) }}" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <select name="class_id"
                    ...
                    @foreach($classes as $class)
                        <option value="{{ $class->id }}"
                            {{ old('class_id', $live->class_id) == $class->id ? 'selected' : '' }}>
                            {{ $class->name }}
                        </option>
                    @endforeach
            ...
            <div>
                ...
                <input type="url" name="stream_url"
                    value="{{ old('stream_url', $live->stream_url) }}"
                    class="w-full mt-2 p-3 rounded-xl border focus:ring-2 focus:ring-indigo-400 outline-none">
                @error('stream_url')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
                    ...
                    <input type="date" name="live_date"
                        value="{{ old('live_date', $live->live_date) }}"
                        class="p-3 rounded-xl border focus:ring-2 focus:ring-indigo-400 outline-none">

                    <input type="time" name="start_time"
                        value="{{ old('start_time', $live->start_time) }}"
                        class="p-3 rounded-xl border focus:ring-2 focus:ring-indigo-400 outline-none">

                    <input type="time" name="end_time"
                        value="{{ old('end_time', $live->end_time) }}"
                        class="p-3 rounded-xl border focus:ring-2 focus:ring-indigo-400 outline-none">
            ...
            <div class="flex gap-3 pt-4">
                ...
                <a href="{{ route('admin.lives.index') }}"
                    class="w-1/2 text-center bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-xl font-semibold transition">
                    Annuler
                </a>

                <button type="submit"
                    class="w-1/2 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold transition">
                    Mettre à jour
                </button>
