...
@section('breadcrumb','Matière → Niveau → Classe → Groupe → Absences')
...
<section class="sp-hero sp-hero-absence"><div class="sp-hero-icon"><i class="bi bi-calendar2-x-fill"></i></div><div class="sp-hero-copy"><span class="sp-kicker">Suivi de présence</span><h2>Mes absences</h2><p>Historique par Matière → Niveau → Classe → Groupe.</p></div><div class="sp-absence-situation {{ $situationClass }}"><i class="bi bi-{{ $situationIcon }}"></i><div><small>Situation actuelle</small><strong>{{ $situation }}</strong></div></div></section>
@if($paths->isNotEmpty())<section class="sp-filter-card"><div class="sp-card-heading"><div class="sp-card-heading-icon red"><i class="bi bi-funnel-fill"></i></div><div><h3>Filtrer mes absences</h3><p>Matière → Niveau → Classe → Groupe.</p></div>@if($hasActiveFilter)<a href="{{ route('student.absences') }}" class="sp-reset-link"><i class="bi bi-arrow-counterclockwise"></i>Tout afficher</a>@endif</div><form method="GET" action="{{ route('student.absences') }}" class="sp-filter-grid">
...
<div class="sp-field"><label>Groupe</label><div class="sp-select-wrap"><i class="bi bi-clock-fill"></i><select name="class_slot_id" id="absenceSlot" disabled><option value="">Tous les groupes</option></select></div></div>
