...
return new class extends Migration
{
    public function up(): void
    {
        ...
        /* Aucune ancienne affectation à convertir sur une installation neuve. */
        if (!DB::table('class_user')->whereIn('class_id', array_keys($mappings))->exists()) {
            return;
        }

        if (DB::getDriverName() === 'sqlite') {
            foreach ($mappings as $oldId => $newId) {
                if (DB::table('class_rooms')->where('id', $newId)->exists()) {
                    DB::table('class_user')
                        ->where('class_id', $oldId)
                        ->where('subject_id', 10)
                        ->update(['class_id' => $newId, 'updated_at' => now()]);
                }
            }
            return;
        }
